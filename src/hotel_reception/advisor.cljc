(ns hotel-reception.advisor
  "ReceptionAdvisor — proposes a hotel-reception operation (check in a
  guest, assist with a guest-service task, disclose a guest's stay
  information, override a reservation-security check) for a
  registered reservation. The advisor is swappable: `mock-advisor`
  ...
  of reservation provenance or disclosure/security-override risk;
  `hotel-reception.governor` is the independent system that decides
  whether the proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :check-in|:assist|:disclose-stay-information|:override-reservation-security-check
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for reservation " (:reservation-id request))})
...
(def ^:private system-prompt
  "You are a hotel-reception advisor. Given a reception operation
