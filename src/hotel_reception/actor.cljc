(ns hotel-reception.actor
  "HotelReceptionActor — the ISCO-08 4224 independent hotel-reception
  actor as a `langgraph.graph/state-graph` (per ADR-2607011000 /
  CLAUDE.md Actors section). One graph run = one reception operation
  ...
  commit a record or dispatch a robot action the HotelReceptionGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [hotel-reception.advisor :as advisor]
            [hotel-reception.governor :as governor]
            [hotel-reception.store :as store]))

(defn build-graph
  "Build a compiled HotelReceptionActor graph. `store` implements
  `hotel-reception.store/Store`. `advisor` implements
  `hotel-reception.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:reservation-id (:reservation-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
