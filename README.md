...
- `src/hotel_reception/store.kotoba` — `Store` protocol + `MemStore`:
  registered reservations, committed records, an append-only audit
  ledger.
- `src/hotel_reception/advisor.kotoba` — `Advisor` protocol;
...
- `src/hotel_reception/governor.kotoba` — `HotelReceptionGovernor/check`:
...
- `src/hotel_reception/actor.kotoba` — `build-graph`, `run-request!`,
